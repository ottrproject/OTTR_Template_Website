...
As an example if you wanted to change the color of the blue line to green you could change `--banner-background-color` in the `assets/style_config_default.css` file from `lightblue` to `lightgreen`. You can also use a hex color code like those that can be found at this [website](https://htmlcolorcodes.com/), such as `#00FF9E` to get a specific shade.

The template also includes examples for centered text and a button on the home page. You can change the button colors with `--button-background-color` and `--button-hover-background-color` in `assets/style_config_default.css`.


```{r, fig.align='center', fig.alt= "Change color of banner", echo = FALSE, out.width="80%"}
knitr::include_graphics("resources/images/banner_color.png")
```
